...
class ModelsManager:
    ...
    def get_path(self, component_type):
        ...
        if (component_type == LABWARE_PLATE):
    ...
    def get_component_models(self, component_type):
        ...
        component_models = [f.split(".")[0] for f in os.listdir(path_of_interest) if (os.path.isfile(os.path.join(path_of_interest, f)) and f.endswith(".json"))] # This returns a list of all the files in the path_of_interest
    ...
    def get_model_parameters(self, component_type: str, model_file_name: str ) -> dict:
        path_to_models_folder = self.get_path(component_type)
        file_path = ""

        if (self.operating_system == "w"):
            file_path = open(path_to_models_folder + "\\" + model_file_name + ".json")
    ...
    def create_plate_model(self, new_model_name, grid, wellplate_well_distance, wellplate_well_depth, nicknames):
        properties = {
            "grid": grid, # This is a list: [rows,columns]
            "wellplate_wellDistance": wellplate_well_distance, # float number
            "well_depth": wellplate_well_depth, # float number or list, to specify a common or specific depth accorss all wellplate_wells respectively
            "nicknames": nicknames # List of lists with strings in them, each sublist holds the nicknames of a row
        }
